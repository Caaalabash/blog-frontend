<template>
  <el-dialog :visible="visible" @close="$emit('close')">
    <!-- 选择收藏夹 -->
    <div class="collect-container fl-column" v-show="!isCreate">
        ...
        <div v-if="collectList.length">
          <div class="list-item fl-row" v-for="item in collectList">
            <div class="text fl-column">
              <span class="text-title">{{ item.collectTitle }}<i v-if="item.collectType==='private'" class="iconfont icon-lock-fill"></i></span>
